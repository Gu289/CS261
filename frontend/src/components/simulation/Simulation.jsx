...
const Simulation = ({ junctionConfig, onSimulationEnd }) => {
  const [trafficData, setTrafficData] = useState([]);
  const [simulationConfig, setSimulationConfig] = useState({
    numLanes: 2,
    trafficLightCycle: 5000, // 5 seconds in milliseconds
  });
  const [simulationStarted, setSimulationStarted] = useState(false);
  
  useEffect(() => {
    // Parse junction config to generate traffic flow data
    if (junctionConfig) {
      const flowData = [];
      
      // Process each direction's flow data
      for (const [fromDir, dirData] of Object.entries(junctionConfig)) {
        if (fromDir === 'numLanes') continue;
        
        // For each direction, process its exit flows
        for (const [toDir, flowRate] of Object.entries(dirData)) {
          if (toDir === 'inbound') continue;
          
          flowData.push({
            from: fromDir,
            to: toDir,
            vph: parseInt(flowRate) // Convert to number to be safe
          });
        }
      }
      
      // Update configuration
      if (junctionConfig.numLanes) {
        setSimulationConfig(prev => ({
          ...prev,
          numLanes: parseInt(junctionConfig.numLanes)
        }));
      }
      
      // Set the processed traffic data
      setTrafficData(flowData);
      setSimulationStarted(true);
    }
  }, [junctionConfig]);

  // Handler for when the simulation ends
  const handleSimulationEnd = (results) => {
    if (onSimulationEnd) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center">
      <h2 className="text-2xl font-bold mb-4 text-white">Traffic Junction Simulation</h2>
      ...
      <div className="mt-4 text-sm text-black">
        <p>This simulation visualizes traffic flow based on your junction configuration.</p>
        <p>The animation uses Bezier curves to smoothly move vehicles through the junction.</p>
      </div>
    </div>
  );
};
