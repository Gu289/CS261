...
const InboundItem = ({ direction, trafficData, handleTrafficChange }) => {
  ...
  return (
    ...
    <input value={trafficData[direction].inbound} onChange={(e) => handleTrafficChange(direction, "inbound", e.target.value)} className='bg-secondary rounded-lg p-2 max-w-20 outline-none border-2'/>
    </div>
  )
}
